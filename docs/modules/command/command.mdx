...
Alternatively, you may also use the `@Arg` annotation directly on the method without the need of ArgProperty, as seen below:
```java
@InjectableComponent
@Command(value = "testfairy", permissionNode = "fairy.test")
public class TestCommand extends BaseCommand {
    ...
    @Command("#")
    public void onNoSubCommand(CommandContext sctx, @Arg("name") String name) {
