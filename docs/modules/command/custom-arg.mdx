...
public class TestCommand extends BaseCommand {
    ...
    public void onNoSubCommand(CommandContext sctx) {
        final String name = sctx.getProperty(this.name);
        final User user = userService.getUser(name);

        if (user == null) {
            sctx.sendMessage(MessageType.ERROR, String.format("Could not find user of name %s!", name));
            return;
        }
...
public class TestCommand extends BaseCommand {
    ...
    @Arg("user")
    private ArgProperty<User> user;

    @Override
    public void onHelp(CommandContext sctx) {
        User user = sctx.getProperty(this.user);
